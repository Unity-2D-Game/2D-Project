...
public class HangingObject : MonoBehaviour
{
    ...
    {
        // 순수한 방향 데이터만 남기기 위해 벡터 정규화 (총알 방향이 너무 짧거나 길어도 일정한 힘을 주도록)
        rigidBody2D.AddForce(forceDirection.normalized * forceMagnitude, ForceMode2D.Impulse);

        // [GameManager 싱글톤 연동 추가]
        // 오브젝트가 총에 맞아서 격렬하게 흔들릴 때마다 게임 매니저에게 알림을 보내도록
        if (GameManager.Instance != null)
        {
            Debug.Log($"[오브젝트 ➔ GameManager] 오브젝트가 타격을 받았습니다.");
        }
    }
